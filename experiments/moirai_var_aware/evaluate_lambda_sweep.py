...
def evaluate_lambda_sweep(input_dir: Path, output_dir: Path, alpha: float = .01) -> None:
    """Write the reviewer-requested lambda-versus-rescaling comparison.

    The scale factor is fit only on lambda=0 validation predictions and then
    applied unchanged to that configuration's test predictions.
    """
    ...
    for key, group in df.groupby(group_cols, sort=True):
        ...
        c = float(c_grid[np.argmin([pinball(yv, factor * pv, alpha) for factor in c_grid])])
    ...
    plt.savefig(output_dir / "lambda_vs_rescaling_pinball.png", dpi=180); plt.close()
