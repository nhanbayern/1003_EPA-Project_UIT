...
os.makedirs('/kaggle/working/results_hybrid/metrics', exist_ok=True)
"""))
...
def evaluate_and_save(model, test_loader, df, index_name, model_name, tier_name):
    ...
    plot_dir = f'/kaggle/working/results_hybrid/visualizations/Predictions/{tier_name}'
...
for tier_name, config in TIERS_CONFIG.items():
    ...
    for fpath in csv_files:
        ...
        loss_dir = f'/kaggle/working/results_hybrid/visualizations/Loss_Curves/{tier_name}'
        ...
        all_metrics_dfs.append(m_metrics)
        
    if len(all_metrics_dfs) > 0:
        final_metrics_df = pd.concat(all_metrics_dfs, ignore_index=True)
        final_metrics_df.to_csv(f'/kaggle/working/results_hybrid/metrics/comparison_metrics_{tier_name}.csv', index=False)
