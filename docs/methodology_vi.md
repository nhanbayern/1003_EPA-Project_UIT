...
Mô hình lai GARCH-LSTM [8] kết hợp tính quy nạp (inductive bias) tài chính của các mô hình kinh tế lượng với khả năng học biểu diễn phi tuyến của mạng nơ-ron. Trong nghiên cứu này, cấu trúc gốc đã được nâng cấp để khắc phục hạn chế về sự bùng nổ phương sai (variance explosion) ở các tầm nhìn dự báo xa.
- **Kiến trúc:** Một cell hồi quy tùy chỉnh (`GARCH_LSTM_Cell`) nhúng trực tiếp phương trình cập nhật phương sai của GARCH vào cơ chế cổng của LSTM, đóng vai trò là bộ trích xuất đặc trưng (feature extractor) trên chuỗi quá khứ. Trạng thái ẩn cuối cùng ($c_{60}$) sau đó được đưa qua một nhánh `VolatilityHead` (mạng MLP nhỏ gồm các lớp Linear, GELU và Softplus) để xuất thẳng ra dự báo phương sai cho nhiều bước cùng lúc.
- **Huấn luyện và Dự báo:** Áp dụng phương pháp dự báo đa bước trực tiếp (Direct Multi-step Forecasting). Mô hình ánh xạ chuỗi đầu vào tới một mảng mục tiêu chứa biến động của toàn bộ 21 bước tương lai. Quá trình tối ưu sử dụng hàm mất mát Student-t NLL tính toán đồng thời trên tất cả các horizon. Ở giai đoạn suy luận (inference), mô hình đưa ra dự báo một lần (one-shot) cho tất cả $h \in \{1, 3, 5, 10, 21\}$, thay vì lặp tự hồi quy (autoregressive), qua đó loại bỏ hoàn toàn sự cộng dồn sai số và nguy cơ mất tính dừng.
